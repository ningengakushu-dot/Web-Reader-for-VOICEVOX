...
- **画面OCR読み上げ（画像・PDF・電子書籍対応）** ★New
  ...
  - **閲覧中のページの上でそのまま**読み上げたい範囲をドラッグ選択すると、文字認識（OCR）→読み上げまで自動で行います。ページの表示やサイズは一切変わらないため、本文を目で追いながら読み上げを聞けます。
...
### v1.3.0
- **画面OCR読み上げ機能を追加しました。** 画像内の文字・PDF・Kindle等の電子書籍・コピー禁止サイトなど、テキストとして選択できない文章も、閲覧中のページ上で範囲をドラッグ選択するだけで文字認識（OCR）→読み上げできます。ページの表示・サイズは変わりません。**縦書き・横書きは自動判定**されるため、縦書きの小説にも対応します。ルビ（ふりがな）を優先して読む設定もオプション画面から任意で有効にできます。
