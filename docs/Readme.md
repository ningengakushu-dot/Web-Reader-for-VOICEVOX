...
### v1.3.0
- **範囲ドラッグ読み上げ機能を追加しました。** 閲覧中のページ上で範囲をドラッグ選択するだけで読み上げます。通常のWebページではページ内の文字をそのまま読み取るため、読み間違いがなく待ち時間もほとんどありません（実測: 実サイト9件で読み取れた率 61.1%→83.6%、待ち時間 10.7秒→44ミリ秒）。画像内の文字・PDF・Kindle等の電子書籍・コピー禁止サイトなど、テキストとして選択できない文章は、自動的に文字認識（OCR）へ切り替わります。ページの表示・サイズは変わりません。**縦書き・横書きは自動判定**されるため、縦書きの小説にも対応します。ルビ（ふりがな）を優先して読む設定もオプション画面から任意で有効にできます。
