# Web Reader for VOICEVOX [v1.2.2]
...
## 動作要件（※重要）
- Google Chrome 119 以降（Offscreen API を使用するため）。
...
### v1.2.2
- 読み上げ中のタブを閉じた場合や、別ページへ移動した場合に音声が鳴り続けてしまう問題を修正しました。
- 拡張機能の更新・再読み込み後、開いたままだったタブでアイコンが無反応になる問題を修正しました（アイコンを撤去し、ページ再読み込みで復帰します）。
- 長い文章の読み上げ中に、アイコンが一時的に待機状態へ戻ってしまう問題を修正しました。
- 拡張機能の更新時に右クリックメニューの登録が失敗し得る問題を修正しました。
- VOICEVOXエンジンが応答しない場合にタイムアウトするようにし、読み上げが無反応のまま止まらないようにしました。
- `document.body` を持たないページ（XML文書など）でも読み上げ機能が停止しないようにしました。
- 設定画面でキャラクター取得に失敗した場合のエラー表示を分かりやすくしました。
- 動作要件を Chrome 119 以降と明示しました。
...
- **ショートカットキーで読み上げが始まらない場合**
  Chromeのアドレスバーに `chrome://extensions/shortcuts` を入力して開き、「Web Reader for VOICEVOX」のショートカット（`Alt+Shift+U`）が割り当てられているかを確認してください。他の拡張機能と競合している場合や未割り当ての場合は、Chromeが許可する範囲で割り当てを見直してください。
